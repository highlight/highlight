...
const buildGridColumns = (
	...
	return columns.map((col) =>
		col.width ? String(col.width) : '1fr',
	)
}
		...
		>
			{showHeader && columns && columns.length > 0 && (
				<HLTable.Head>
					<HLTable.Row gridColumns={gridColumns}>
						{columns.map((col, i) => (
							<HLTable.Header key={String(col.key ?? col.dataIndex ?? i)}>
								{col.title}
							</HLTable.Header>
						))}
					</HLTable.Row>
			...
			<HLTable.Body>
				{loading ? (
					...
					renderEmptyComponent ? (
						<HLTable.FullRow>{renderEmptyComponent}</HLTable.FullRow>
					) : null
				) : (
					dataSource.map((record, i) => {
						...
						return (
							...
							>
								{columns?.map((col, j) => (
									<HLTable.Cell
										key={String(col.key ?? col.dataIndex ?? j)}
										style={
											col.align
												? { textAlign: col.align }
												: undefined

...
const TimelineIndicatorsBarGraph = React.memo(
	({ selectedTimelineAnnotationTypes, width }: Props) => {
		...
		const [hasActiveScrollbar, setHasActiveScrollbar] =
		...
		const onVisibilityChange = useCallback(
			(event: MouseEvent) => {
				...
				const isInsideViewport = isInsideElement(
					event,
					viewportRef.current,
				)
				...
				setShowIndicatorText(
					isDragging ||
						isInsideElement(event, timeIndicatorRef.current),
				)
			},
			[isDragging, moveTime],
		)
		...
		useLayoutEffect(() => {
			const left = Number.isFinite(zoomStart)
			...
			for (let idx = 0; idx <= numTicks; ++idx) {
				...
				if (idx !== numTicks) {
					for (
						...
						const isMid =
							minorIdx === mid && MINOR_TICK_COUNT % 2 === 1
			...
			<div className={style.timelineContainer} style={{ width }}>
				...
				>
					...
					{adjustedInactivityPeriods.map((inactive, idx) => {
						const width =
							(inactive[1] / canvasDuration) * canvasWidth
						const left =
							TIMELINE_MARGIN +

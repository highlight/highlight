...
A naive approach to tracing creates a new trace for every service that is used to resolve a request.

Distributed tracing connects traces across multiple services to create a type of distributed "call stack".

For instance, a React web client can start the trace, which can be continued in a Next.js API function. Any subsequent HTTP call from the Next.js function should also be connected as part of the same trace.

Service-oriented architectures often create a web of HTTP calls, all of which should be connected back to the initial request.

![Distributed tracing flame graph](/images/blog/tracing-distributed-systems-in-nextjs/trace-screenshot-with-callouts.png)

## Approach

The following example of Distributed Tracing is extracted from this repo:

**[GitHub: Distributed Tracing Example](https://github.com/highlight/distributed-tracing-example)**

We will demonstrate how to use Highlight to connect multiple services together by passing HTTP headers.

- The web client initiates an HTTP `Fetch` request, passing the `x-highlight-request` header which contains a generated `sessionId` and `requestId`.
- A Next.js API function receives that request, associates the `sessionId` and `requestId` to it's local trace, then passes the `x-highlight-request` header to a third service, this time written in Golang.
- The Golang service receives the `x-highlight-request` header and associates it to the local trace.
...
Use the browser's `Fetch` API to call a Next.js API function. 

Highlight monkey patches `window.fetch` and adds the `x-highlight-request` header automatically.
...
We're using the [Highlight chi SDK](https://www.highlight.io/docs/getting-started/backend-sdk/go/chi).
...
All three services, the web client, the Next.js API function, and the Golang backend, feed their data to the same Highlight project.

The result can be seen on the Traces tab of the Highlight dashboard.
...
- `go-custom-span` is a custom span that we used in our example code for demonstration purposes. It wraps a call to `time.Sleep(1 * time.Second)`.

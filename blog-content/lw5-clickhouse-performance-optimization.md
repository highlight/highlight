...
merging parts. For instance, we found that switching our `ORDER BY Timestamp` to an `ORDER BY toStartOfSecond(‘minute’,
...
setting `min_bytes_for_full_part_storage=4294967296` will ensure most parts use `Packed` storage which may
be more efficient for future large merges.

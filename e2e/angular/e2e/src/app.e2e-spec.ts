...
describe('Getting Started', () => {
	const pageElements = {
		...
		topBarCheckoutLink: element(
			by.cssContainingText('app-root app-top-bar a', 'Checkout'),
		),
	...
	describe('Product Details', () => {
		...
		it('should add the product to the cart', async () => {
			...
			const cartItems = await element.all(
				by.css('app-root app-cart div.cart-item'),
			)
	...
	describe('Cart', () => {
		...
		it('should go through the checkout process', async () => {
			...
			const cartMessages = logs.filter(({ message }) =>
				message.includes('Your order has been submitted'),
			)

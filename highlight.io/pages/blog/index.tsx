...
import { getTagUrl, PostTag, SidebarTag, Tag, TagTab } from '../../components/Blog/Tag'
...
export async function loadPostsFromHygraph(tag?: string) {
  ...
  return posts.filter((p) => (tag ? p.tags_relations.some((t) => t.slug === tag) : true)) as Post[]
}
...
export const Blog = ({
  ...
  const currentTag: Omit<Tag, 'posts'> = tags.find(({ slug }) => slug === currentTagSlug) ?? allTag

  const [searchQuery, setSearchQuery] = useState<string>('')
  const debouncedSearchQuery = useDebounce(searchQuery, 300)
  const itemsPerPage = 4

  const shouldFeature = !debouncedSearchQuery && currentTag.slug === allTag.slug && page <= 1
      ...
      <main>
        <div className="flex flex-row w-full gap-8 my-20 desktop:max-w-[1100px] mx-auto items-start px-6">
          <div /* Main Side */ className="flex flex-col flex-1 w-full gap-11">
            <div /* Category Description */ className="flex flex-col items-start gap-5">
              <h3>{isStartupStack ? 'Welcome to the Startup Stack!' : currentTag.name}</h3>
              {isStartupStack ? (
                <Typography className={styles.copyOnDark} type="copy1">
                  This is where we talk about the tools and tech you can use to build your next Startup! Read through
                  our episodes below or find us{' '}
                  <Link href="https://www.youtube.com/channel/UCATzQs36Mo7Cezt5Ij9ayZQ">on YouTube</Link>.
                </Typography>
              ) : (
            ...
            <div /* Search and Posts */ className="flex flex-col gap-6">
              <div /* Tags Tabs */ className="flex gap-8 overflow-x-scroll scrollbar-hidden">
                {shownTags.map((tag) => (
                  <TagTab {...tag} key={tag.slug} current={currentTag.slug === tag.slug} />
                ))}
              </div>
              <div /* Search and Pagination */ className="flex flex-col justify-between w-full gap-4 mobile:flex-row">
                <div /* Search */
                ...
                >
                  ...
                  <input
                    type="text"
                    placeholder={
                      currentTag.slug === 'all' ? 'Search all posts...' : `Search all ${currentTag.name} posts...`
                    }
                    value={searchQuery}
              ...
              {displayedPosts.length < 1 && (
                <Typography type="copy2" className="my-10 text-center text-copy-on-light">
                  No Posts Found
                </Typography>
              )}
              {displayedPosts.length > 0 && !debouncedSearchQuery && (
                <PageController page={page} count={filteredPosts.length / itemsPerPage} tag={currentTag.slug} />
              )}
            </div>
          </div>
        </div>
...
function PageController({ page, count, tag }: { page: number; count: number; tag: string }) {
  return (
    ...
    <>
      <div className="hidden mobile:block">
        ...
        >
          {post?.image?.url && (
            <Image
              style={{ borderRadius: 8, border: '2px solid #30294E', height: 100 * 1.5, width: 190 * 1.5 }}
              src={post.image?.url}
              alt={'podcast image'}
              height={100}
              width={190}
      ...
      <div className={classNames('mobile:hidden block')}>
        ...
        >
          <div>
            ...
            <div className="mt-6">{post.author && <PostAuthor {...post.author} hidePhoto />}</div>
          </div>
        </div>
      </div>
    </>
...
const PostItem = ({ post, feature: featured = false }: { post: Post } & { feature?: boolean }) => {
  const firstTag = post.tags_relations[0]
  return (
    <>
      <div className={classNames(postItemStyle, featured && 'shadow-[8px_8px_0_0_#5420D1]', 'hidden mobile:block')}>
        ...
        <h5 className="mt-1">{post.title}</h5>
        <div className="relative mt-3 pointer-events-none">{post.author && <PostAuthor {...post.author} />}</div>
      ...
      <div className={classNames(postItemStyle, featured && 'shadow-[8px_8px_0_0_#5420D1]', 'mobile:hidden block')}>
        <Link href={`/blog/${post.slug}`} className="absolute inset-0">
          <div />
        </Link>
        {firstTag && (

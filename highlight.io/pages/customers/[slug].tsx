        ...
        <div className={style.detailsLayout}>
          ...
          <div className={style.caseContent}>
            ...
            <div className={style.casePageLinks}>
              {previousCase ? (
                <PageLink label="Previous Customer" slug={previousCase.slug} logo={previousCase.companyLogo?.url} />
              ) : (
                <div />
              )}
              {nextCase && <PageLink label="Next Customer" slug={nextCase.slug} logo={nextCase.companyLogo?.url} />}
            </div>
          </div>
          <div className={style.caseCustomerDetails}>
            ...
            <div className={style.caseDetailsBody}>
              <CustomerDetailsSection label="About the company" body={customer.about} />
              <CustomerDetailsSection label="Founded" body={new Date(customer.founded).getFullYear().toString()} />
              <CustomerDetailsSection
                label="Using Highlight since"
                body={new Date(customer.usingHighlightSince).toLocaleDateString(undefined, {
                  month: 'short',
                  year: 'numeric',
                })}
              />
...
const PageLink = ({ label, slug, logo }: { label: string; slug: string; logo?: string }) => (
  <Link href={`/customers/${slug}`}>
    <div className={style.casePageLink}>
      ...
      {logo && (
        <Image src={logo} width={187} height={32} objectFit="contain" objectPosition="left" alt={`${slug} logo`} />
      )}
    </div>
  </Link>
)

const CustomerDetailsSection = ({ label, body }: { label: string; body: string }) => (

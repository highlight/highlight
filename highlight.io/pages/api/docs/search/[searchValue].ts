...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const paths: SearchResult[] = await Promise.all(
    docPaths.map(async (doc) => {
      const { content } = await readMarkdown(fsp, path.join(doc?.total_path || ''))
      return {
  ...
  const filteredResults = paths.filter(
    (path) =>
      (path.title.toLowerCase().includes(searchValue) || path.content.toLowerCase().includes(searchValue)) &&
      !path.indexPath,
  )
  const searchResults = filteredResults.map((result) => {
    ...
    return {
      ...result,
      content: `${firstOccurence - SEARCH_RESULT_BLURB_LENGTH > 0 ? '...' : ''}${parsedContent.slice(

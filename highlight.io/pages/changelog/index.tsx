...
import { ChangelogEntry, Entry } from '../../components/Changelog/ChangelogEntry/ChangelogEntry'
...
const Changelog = ({ changelogs }: { changelogs: Array<never> }) => {
  ...
  useEffect(() => {
    setCurrentItems(
      changelogs.slice(ITEMS_PER_PAGE * (currentPage - 1), Math.min(ITEMS_PER_PAGE * currentPage, changelogs.length)),
    )
  }, [currentPage, changelogs])

  return (
    <>
      <Meta title={'Highlight Changelog'} description={'Stop debugging in the dark.'} />

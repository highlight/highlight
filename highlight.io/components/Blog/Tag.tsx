...
import { HiChartPie, HiCog, HiCollection, HiPlay, HiTag, HiTerminal } from 'react-icons/hi'
...
export const getTagUrl = (slug: string) => (slug === 'all' || !slug ? `/blog` : `/blog/tag/${slug}`)

function TagIcon({ slug, className }: { slug: Tag['slug']; className?: string }) {
...
export function SidebarTag({ name, slug, current }: { name: Tag['name']; slug: Tag['slug']; current?: boolean }) {
  return (
    <Link href={getTagUrl(slug)}>
...
export function TagTab({ name, slug, current }: { name: Tag['name']; slug: Tag['slug']; current?: boolean }) {
  ...
  const tabColorStyle = classNames(
    current ? 'text-highlight-yellow border-highlight-yellow' : 'text-copy-on-dark border-transparent',
  )
  ...
  return (
    <Link href={getTagUrl(slug)}>
      <div className={classNames(tabBaseStyle, tabColorStyle)}>
        <span
          className={classNames(
            current ? 'text-highlight-yellow' : 'opacity-70 group-hover:opacity-100 transition-opacity',
            'mt-1',
          )}
        >
          <TagIcon slug={slug} />

...
const SocialLink = ({ href, icon }: { href: string; icon: ReactElement }) => (
  <a href={href} className="transition-colors text-copy-on-dark" target={'_blank'} rel="noreferrer">
    {icon}
  </a>
)
    ...
    <div className="flex gap-3">
      {!hidePhoto && (
        <div className="overflow-hidden rounded-full w-12 h-12 border-solid border-[3px] border-divider-on-dark relative">
          <Image src={profilePhoto.url ?? ''} layout="fill" alt="author picture" objectFit="cover" />
        </div>
      )}
      <div className="flex flex-col items-start">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2 pointer-events-auto">
            {personalWebsiteLink && <SocialLink href={personalWebsiteLink} icon={<HiGlobeAlt />} />}
            {twitterLink && <SocialLink href={twitterLink} icon={<FaTwitter />} />}
            {githubLink && <SocialLink href={githubLink} icon={<FaGithub />} />}
            {linkedInLink && <SocialLink href={linkedInLink} icon={<FaLinkedin />} />}
          </div>
        </div>

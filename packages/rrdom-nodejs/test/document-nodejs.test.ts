...
describe('RRDocument for nodejs environment', () => {
	describe('RRDocument API', () => {
		...
		it('querySelectorAll querying tag name', () => {
			...
			expect(
				(rrdom.querySelectorAll('H1')[0] as RRElement).tagName,
			).toEqual('H1')
			expect(rrdom.querySelectorAll('H1')[1]).toBeInstanceOf(RRElement)
			expect(
				(rrdom.querySelectorAll('H1')[1] as RRElement).tagName,
			).toEqual('H1')
		})
		...
		it('querySelectorAll querying class name', () => {
			...
			for (let element of rrdom.querySelectorAll('.\\:hover')) {
				...
				expect((element as RRElement).classList.classes).toContain(
					':hover',
				)
			}
		})
		...
		it('querySelectorAll querying id', () => {
			for (let query of ['#block1', '#block2', '#block3']) {
				...
				const targetElement = rrdom.querySelectorAll(
					query,
				)[0] as RRElement
				expect(targetElement.id).toEqual(
		...
		it('querySelectorAll', () => {
			expect(
				rrdom.querySelectorAll('link[rel="stylesheet"]').length,
			).toEqual(1)
	...
	describe('RRElement API', () => {
		...
		it('can get attribute', () => {
			...
			expect(
				rrdom.getElementsByTagName('DIV')[0].getAttribute('id'),
			).toEqual('block1')
			expect(
				rrdom.getElementsByTagName('div')[0].getAttribute('iD'),
			).toEqual('block1')
		...
		it('get nextElementSibling', () => {
			...
			expect(rrdom.documentElement!.firstElementChild!.tagName).toEqual(
				'HEAD',
			)
			expect(
		...
		it('style element', () => {
			...
			expect(rrdom.getElementsByTagName('style')[0].tagName).toEqual(
				'STYLE',
			)

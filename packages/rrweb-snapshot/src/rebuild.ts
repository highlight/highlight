			...
			for (const name in sortAttributes(n.attributes)) {
				...
				if (!name.startsWith('rr_')) {
					...
					if (tagName === 'canvas' && name === 'rr_dataURL') {
						...
						image.onload = () => {
							const ctx = (node as HTMLCanvasElement).getContext(
								'2d',
							)
							if (ctx) {
								ctx.drawImage(
						...
						if ((node as unknown as RRCanvasElement).RRNodeType)
							(node as unknown as RRCanvasElement).rr_dataURL =
								value
						...
						switch (value) {
							case 'played':
								;(node as HTMLMediaElement)
									.play()
									.catch((e) =>
										console.warn('media playback error', e),
									)
								break
							case 'paused':
	...
	if (n.type === NodeType.Document) {
		...
		if (
			...
			if (
				...
				n.childNodes[0].attributes.xmlns ===

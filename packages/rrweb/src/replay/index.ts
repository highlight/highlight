...
export class Replayer {
		...
		if (firstFullsnapshot) {
			setTimeout(() => {
				...
				this.rebuildFullSnapshot(
					firstFullsnapshot as fullSnapshotEvent & {
						timestamp: number
					},
				)
			...
			case EventType.IncrementalSnapshot:
				castFn = () => {
					...
					if (
						...
						for (const _event of this.service.state.context
							.events) {
							...
							if (this.isUserInteraction(_event)) {
								if (
									...
									SKIP_TIME_THRESHOLD *
										this.speedService.state.context.timer
											.speed
								) {
						...
						if (this.nextUserInteractionEvent) {
							const skipTime =
								this.nextUserInteractionEvent.delay! -
								event.delay!
							const payload = {
								speed: Math.min(
	...
	private insertStyleRules(
		...
		if (this.usingVirtualDom) {
			...
			;(documentElement as RRElement)!.insertBefore(
				styleEl,
				head as RRElement,
			)
	...
	private attachDocumentToIframe(
		...
		buildNodeWithSN(mutation.node, {
			...
			afterAppend: (builtNode) => {
				this.collectIframeAndAttachDocument(collected, builtNode)
				const sn = (mirror as TMirror).getMeta(
	...
	private waitForStylesheetLoad() {
		...
		if (head) {
			...
			head.querySelectorAll('link[rel="stylesheet"]').forEach(
				(css: HTMLLinkElement) => {
					if (!css.sheet) {
				...
				const commands = await Promise.all(
					data.commands.map(async (c) => {
						const args = await Promise.all(
							c.args.map(
								deserializeArg(this.imageMap, null, status),
							),
						)
						return { ...c, args }
					}),
					...
					case MouseInteractions.TouchEnd:
						if (isSync) {
							...
							if (d.type === MouseInteractions.Click) {
								...
								d.type === MouseInteractions.TouchStart
							) {
		...
		const appendNode = (mutation: addedNodeMutation) => {
			...
			if (mutation.node.isShadow) {
				...
				if (!hasShadowRoot(parent)) {
					;(parent as Element | RRElement).attachShadow({
						mode: 'open',
					})
					parent = (parent as Element | RRElement).shadowRoot! as
						| Node
						| RRNode
			...
			if (
				...
				if (parent === targetDoc) {
					while (targetDoc.firstChild) {
						;(targetDoc as TNode).removeChild(
							targetDoc.firstChild as TNode,
						)
					}
				}
		...
		d.attributes.forEach((mutation) => {
			...
			for (const attributeName in mutation.attributes) {
				if (typeof attributeName === 'string') {
					...
					if (value === null) {
						;(target as Element | RRElement).removeAttribute(
							attributeName,
						)
						...
						for (const s in styleValues) {
							if (styleValues[s] === false) {
								...
								const svp = styleValues[
									s
								] as styleValueWithPriority

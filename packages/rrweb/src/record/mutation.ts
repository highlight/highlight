...
export default class MutationBuffer {
	...
	public emit = () => {
		...
		const pushAdd = (n: Node) => {
			...
			while (
				(rootShadowHost?.getRootNode?.() as ShadowRoot | undefined)
					?.host
			)
				rootShadowHost =
					(rootShadowHost?.getRootNode?.() as ShadowRoot | undefined)
			...
			const sn = serializeNodeWithId(n, {
				...
				onSerialize: (currentN) => {
					if (isSerializedIframe(currentN, this.mirror)) {
						this.iframeManager.addIframe(
							currentN as HTMLIFrameElement,
						)
					}
					if (isSerializedStylesheet(currentN, this.mirror)) {
						this.stylesheetManager.addStylesheet(
				...
				onIframeLoad: (iframe, childSn) => {
					this.iframeManager.attachIframe(
						iframe,
						childSn,
						this.mirror,
					)
				...
				onStylesheetLoad: (link, childSn) => {
					this.stylesheetManager.attachStylesheet(
						link,
						childSn,
						this.mirror,
					)
	...
	private processMutation = (m: mutationRecord) => {
		...
		switch (m.type) {
			...
			case 'attributes': {
				...
				let value = (m.target as HTMLElement).getAttribute(
					m.attributeName!,
				)
				...
				if (m.attributeName === 'style') {
					...
					const styleObj = item.attributes
						.style as styleAttributeValue
				...
				m.removedNodes.forEach((n) => {
					...
					if (this.addedSet.has(n)) {
						...
						this.removes.push({
							...
							id: nodeId,
							isShadow:
								isShadowRoot(m.target) &&
								isNativeShadowDom(m.target)
									? true
									: undefined,
						})
					}

...
func (c *Client) Search(indexes []Index, projectID int, query string, options SearchOptions, results interface{}) (*SearchResponse, error) {
	...
	req := fmt.Sprintf(`{"_source": {"excludes": [%s]}, "size": %d, "query": %s%s, "track_total_hits": %s}`,
		excludesStr, count, q, sort, trackTotalHits)
	content := strings.NewReader(req)
...
func (c *Client) formatSearchResponse(res []byte, results interface{}) (*SearchResponse, error) {
	...
	if err := json.Unmarshal(res, &response); err != nil {
		return nil, e.Wrap(err, "failed to unmarshal scroll response")
	}
	...
	if err != nil {
		return nil, e.Wrap(err, "failed to re-marshal sources")
	}

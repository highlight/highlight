	...
	query := fmt.Sprintf(`
      ...
      union(tables: [
		...
		do(q:0.99)
		  ...
		  |> sort()
  `, r.TDB.GetBucket(strconv.Itoa(projectID)), params.DateRange.StartDate.Format(time.RFC3339), params.DateRange.EndDate.Format(time.RFC3339), timeseries.Metrics, metricName, projectID, div)
	...
	`, r.TDB.GetBucket(strconv.Itoa(projectID)), params.DateRange.StartDate.Format(time.RFC3339), params.DateRange.EndDate.Format(time.RFC3339), timeseries.Metrics, metricName, projectID, histogramPayload.P1*div, bucketSize*div, numBuckets, div)
...
func (r *queryResolver) NetworkHistogram(ctx context.Context, projectID int, params modelInputs.NetworkHistogramParamsInput) (*modelInputs.CategoryHistogramPayload, error) {
	...
	query := fmt.Sprintf(`
		from(bucket: "%s")
          ...
          |> limit(n: 10)
		  |> yield(name: "count")
	`, r.TDB.GetBucket(strconv.Itoa(projectID)), days, timeseries.Metrics, projectID, extraFiltersStr, params.Attribute.String())

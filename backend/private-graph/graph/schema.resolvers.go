...
// StreamLogs is the resolver for the stream_logs field.
func (r *subscriptionResolver) StreamLogs(ctx context.Context, projectID int, params modelInputs.LogsParamsInput, after *string, before *string, at *string) (<-chan *modelInputs.LogsConnection, error) {
	project, err := r.isAdminInProject(ctx, projectID)
	if err != nil {
		return nil, e.Wrap(err, "error querying project")
	}

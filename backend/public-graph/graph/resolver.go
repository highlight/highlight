...
func (r *Resolver) InitializeSessionImpl(ctx context.Context, input *kafka_queue.InitializeSessionArgs) (*model.Session, error) {
	...
	defer func() {
		err := r.Redis.SetSessionInitialized(ctx, input.SessionSecureID, false)
		if err != nil {
			log.Error(e.Wrapf(err, "failed to clear session initialized flag for %s", input.SessionSecureID))
		}
	}()

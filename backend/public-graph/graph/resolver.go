...
func (r *Resolver) AddTrackProperties(ctx context.Context, sessionID int, events *parse.ReplayEvents) error {
	...
	for _, event := range events.Events {
		if event.Type == parse.Custom {
			...
			if err := json.Unmarshal(event.Data, &dataObject); err != nil {
			...
			if err := json.Unmarshal(payload, &propertiesObject); err != nil {
				return e.Wrap(err, "error deserializing track event properties")
			}

			log.Warnf("%v", propertiesObject)
			for k, v := range propertiesObject {
				formattedVal := fmt.Sprintf("%.*v", SESSION_FIELD_MAX_LENGTH, v)
				strOrJson, err := strconv.Unquote(formattedVal)
				if err != nil {
					formattedVal = strOrJson
				}

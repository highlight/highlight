...
func (w *Worker) Start() {
	for {
		...
		if err := w.R.DB.Where("(payload_updated_at < ? OR payload_updated_at IS NULL) AND (processed = ? OR processed IS NULL)", thirtySecondsAgo, false).Find(&sessions).Error; err != nil {
